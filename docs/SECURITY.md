...
- Paddle webhooks require a timestamped HMAC signature and are idempotent.
...
- Production startup must provide a strong `JWT_SECRET`, Resend credentials, Paddle secrets, TLS termination, and private PostgreSQL/Redis networking.

Before launch, perform dependency, entitlement bypass, IDOR, rate-limit, malformed media, object retention, account deletion, and webhook replay testing. Add edge rate limiting and a malware scanning job before accepting uploads from untrusted sources.
